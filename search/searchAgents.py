...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            x,y = state[0]
            dx, dy = Actions.directionToVector(action)
            location = (int(x + dx), int(y + dy))
            if not self.walls[location[0]][location[1]]:
                if location in self.corners and location not in state[1] :
                    cornerholder = list(state[1])
                    cornerholder.append(location)
                    nextState = (location, tuple(cornerholder))
                else :
                    nextState = (location, state[1])
...
def cornersHeuristic(state, problem):
    ...
    for count in range(0,len(problem.corners)- len(state[1])):
        nearestcornerdistance = 999999999
        nearestcorner = None
        for corner in problem.corners :
            if corner not in state[1] and corner not in seencorners :
                newdistance = abs(corner[0] - currentlocation[0]) + abs(corner[1] - currentlocation[1])
                if newdistance < nearestcornerdistance :
                    nearestcornerdistance = newdistance
                    nearestcorner = corner
        totalmindistance = totalmindistance + nearestcornerdistance
        currentlocation = nearestcorner
        seencorners.append(nearestcorner)
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    import math
    rows = foodGrid.height
    collums = foodGrid.width
    foodlocations = list()
    for y in range(0,collums):
        currentrow =  foodGrid.__getitem__(y)
    ...
    return math.ceil(totalmindistance / 1.6)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        return self.food.__getitem__(x)[y]
